...
export const register = (userInfo) => async (dispatch) => {
    try {
        const response = await apiCall(urls.register, 'POST', userInfo);
...
export const login = (loginData) => async (dispatch) => {
    try {
        const response = await apiCall(urls.logIn, 'POST', loginData);
...
export const logout = () => async (dispatch) => {
    try {
        const response = await apiCall(urls.logOut, 'POST');
        dispatch({
            type: LOGOUT,
        });
        return response;
